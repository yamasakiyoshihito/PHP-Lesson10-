...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (strlen(trim($_POST["password"] ?? "")) == 0) { //入力されているのか確認
        $errors = "メールアドレスとパスワードを正しく入力してください。";
    }
    ...
    if (empty($errors)) {
        ...
        try {
            ...
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION); //エラーモードを「例外」に」設定
        ...
        //ユーザー情報が取り出せた かつ パスワードが一致すれば、セッションを値を代入し、マイページへ遷移
        if ($user && password_verify($input["password"], $user["password"])) {
            ...
            //「ログイン情報を保持する」にチェックあれば、セッションにセットする
